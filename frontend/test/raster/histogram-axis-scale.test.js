...
import { defaultHistogramAxisOptions, resolveHistogramAxis } from "../../src/raster/histogram-axis-scale.js";
...
test("bar-height values use percentage units and percentiles exclude empty bins", () => {
    const fixed = resolveHistogramAxis(frequency, options({ bounds: "values", minimum: "0", maximum: "5" }));
    assert.equal(fixed.position(5), 1);
    assert.match(fixed.notice, /clipped/);
    const quantile = resolveHistogramAxis(frequency, options({ bounds: "percentiles", minimum: "25", maximum: "75" }));
    assert.equal(quantile.minimum, 20);
    assert.equal(quantile.maximum, 45);
    const values = resolveHistogramAxis(distribution, options({ bounds: "percentiles", minimum: "0", maximum: "50" }));
...
test("log coordinates and clipping retain original bins without an offset", () => {
    ...
    const axis = resolveHistogramAxis(data, options({ scale: "log" }));
    ...
    const logFrequency = resolveHistogramAxis(frequency, options({ scale: "log" }));
...
test("invalid, empty and degenerate ranges fail locally", () => {
    for (const [minimum, maximum] of [["", "5"], ["4", "4"], ["8", "2"], ["NaN", "4"], ["0", "Infinity"]]) {
        assert.throws(() => resolveHistogramAxis(frequency, options({ bounds: "values", minimum, maximum })), RangeError);
    }
    assert.throws(() => resolveHistogramAxis(frequency, options({ bounds: "values", minimum: "-1", maximum: "5" })), /between 0 and 100/);
    assert.throws(() => resolveHistogramAxis(frequency, options({ bounds: "percentiles", minimum: "1", maximum: "101" })), /Percentiles/);
    assert.throws(() => resolveHistogramAxis({ ...frequency, counts: [0, 0, 0, 0], total: 0 }), /No sampled/);
    assert.throws(() => resolveHistogramAxis({ ...frequency, counts: [0, 0, 0, 1], total: 1 },
        options({ bounds: "percentiles", minimum: "0", maximum: "100" })), /same value/);
    assert.throws(() => resolveHistogramAxis({ edges: [-3, -2, -1], counts: [1, 1], total: 2 }, options({ scale: "log" })), /positive edges/);
    assert.throws(() => resolveHistogramAxis(distribution,
        options({ scale: "log", bounds: "values", minimum: "0", maximum: "4" })), /positive minimum/);
    assert.equal(resolveHistogramAxis({ edges: [1, 2], counts: [1], total: 1 }).maximum, 2);
});
...
test("percentiles resolve with each sample while explicit limits stay fixed", () => {
    ...
    assert.equal(resolveHistogramAxis(next, fixed).minimum, resolveHistogramAxis(frequency, fixed).minimum);
    assert.notEqual(resolveHistogramAxis(next, percentile).minimum, resolveHistogramAxis(frequency, percentile).minimum);
});
